...
import { RANKS, RANK_COLORS, Rank, Player, Custom, isBRMode } from '../lib/types';
...
export default function EditCustomDialog({ custom, onClose, onSave, onDelete, onDuplicate }: Props) {
  const isBR = isBRMode(custom.mode);
    ...
    >
      <div className="mx-4 w-full max-w-lg rounded-2xl bg-white shadow-2xl dark:bg-[#20213a]">
        ...
        <div className="space-y-5 px-6 pb-2">
          ...
          {isBR ? (
            /* バトルロイアルモードはチーム単位で参加登録するため、詳細画面で管理する */
            <div>
              <p className="mb-1.5 block text-sm font-medium text-zinc-700 dark:text-zinc-300">参加チーム</p>
                ...
                <ul className="max-h-84 space-y-1.5 overflow-y-auto">
                  {custom.brTeams.map((team) => (
                    ...
                    >
                      ...
                      <span className="text-xs text-zinc-400">{team.players.length}人</span>
                    </li>
                  ))}
                </ul>
              )}
              <p className="mt-2 text-xs text-zinc-400">※ チームやプレイヤーの追加・編集はカスタム詳細画面から行ってください。</p>
            </div>
          ) : (
            <>
              {/* Add player */}
